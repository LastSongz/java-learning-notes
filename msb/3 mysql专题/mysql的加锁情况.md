...
出现其他行的临键锁的原因是：为了防止幻读，如果不添加Next-Key Lock锁，这时若有其他会话执行insert语句，则都会造成幻读。

innodb无法确认id = 10是唯一一行，因此他不会把where id = 10视为单行精准匹配，而是视为匹配所有id=10的行的范围查询，为了防止幻读（其他事务插入新的id=10的行），innodb需要锁定id=10这行，同时锁定id = 10所在的整个临键锁区间（-∞，10]，但因为id不是索引，innodb无法仅锁定（-∞，10]，需要通过隐式聚簇索引来加锁，最终导致间
...
通过上述信息可以看到，首先对表添加IX锁，然后对supremum添加临键锁，对name索引列添加临键锁，对主键索引添加X记录锁，此处需要注意的是，执行语句走的索引是name，所以会给name添加临键锁，如果强制指定主键索引的话，那么就会给主键添加临键锁，name的索引上无锁
...
在上述的案例中，我们演示了在不同的隔离级别下加锁的情况，下面进行总结：

1、在RC隔离级别下，无论你的索引是什么，无论进行什么样的查询，只会存在记录锁

2、在RR隔离级别下，加锁的情况相对复杂，需要确认的核心点是当前表中有哪些索引，以及执行查询的过程中会命中哪些索引，简单概括为：

- RR 级别加锁的 “底线” 是临键锁，只有 “主键 / 唯一索引的等值命中查询” 才会降级为仅记录锁；
- 索引决定加锁范围：唯一索引最精准，普通索引锁索引行 + 间隙，无索引锁全表；
- 未命中的唯一索引等值查询，只会锁 “不存在的记录对应的间隙”，不会锁记录。

上述的加锁规则只是为了方便大家的理解和记忆，设计到具体的情况，最好还是通过存储引擎的状态来判断具体的加锁情况

想要明确看到加锁的情况，需要在mysql中开启2个参数，分别是：
